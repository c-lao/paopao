...
const heroImage = `${BASE_URL}Frame 427319301.png`;
...
const Hero: React.FC = () => {
  ...
  useEffect(() => {
    ...
    tl.to([titleRef.current, subtitleRef.current], {
      ...
      // 第二组：描述文字和按钮同时出现
      .to(
        [descriptionRef.current, buttonsRef.current],
        {
          opacity: 1,
          y: 0,
          duration: 1,
    ...
    return () => {
      // 清理事件监听器和动画
      const buttons = [iosButtonRef.current, androidButtonRef.current].filter(
        Boolean
      );
        ...
        >
          ...
          <div className="flex justify-center lg:justify-end relative order-1 lg:order-2 w-full max-w-full pb-6 pt-2">
            ...
            <div
              ...
              style={{
                background:
                  'radial-gradient(ellipse at 30% 20%, rgba(99, 102, 241, 0.1) 0%, transparent 60%)',
                filter: 'blur(15px)',
                transform: 'scale(1.1)',
              }}
            />
